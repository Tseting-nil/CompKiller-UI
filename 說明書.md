# UI庫調用說明書

**版本:** 2026/02/25
**最後更新:** 2026/02/25

## 載入函式庫

```lua
local Compkiller = loadstring(game:HttpGet("https://raw.githubusercontent.com/Tseting-nil/CompKiller-UI/refs/heads/main/src/UI_source.lua"))();

```

## 建立視窗 - 參數說明
### new 參數
| 參數             | 類型           | 說明                                                                   |
| ---------------- | -------------- | ---------------------------------------------------------------------- |
| `Name`           | string         | 左上角顯示名稱                                                         |
| `Keybind`        | string         | 電腦端鍵盤快速開關UI                                                   |
| `Logo`           | string,Boolean | UI左上角Icon  {ID , 顯示}                                              |
| `Scale`          | function       | 根據設備自動選擇UI大小 (別動)                                          |
| `TextSize`       | number         | 自訂UI文字大小（全域）                                                 |
| `default_Toggle` | string         | 開關預設值加載行為：`"on"` 加載時執行 Callback，`"off"` 不執行（預設） |

```lua
local Window = Compkiller.new({
    Name = "Compkiller UI",
    Keybind = "LeftAlt",
    Logo = {"rbxassetid://15307540148", true},
    -- 根據設備自動選擇 UI 尺寸：手機用小介面，電腦/平板用大介面
    Scale = Compkiller:_UseSmallUI() and Compkiller.Scale.Mobile or Compkiller.Scale.Window,
    TextSize = 15,
    default_Toggle = "off", -- "on" = 加載時執行 Callback, "off" = 不執行（預設）
});
```

## 視窗控制方法

### Window:SetVisible(bool)
控制UI的顯示/隱藏狀態。

| 參數   | 類型    | 說明                          |
| ------ | ------- | ----------------------------- |
| `bool` | boolean | `true` 顯示UI，`false` 隱藏UI |

```lua
-- 隱藏UI
Window:SetVisible(false)

-- 顯示UI
Window:SetVisible(true)
```

### Window:Toggle(bool)
切換UI的開關狀態。

| 參數   | 類型    | 說明                          |
| ------ | ------- | ----------------------------- |
| `bool` | boolean | `true` 開啟UI，`false` 關閉UI |

```lua
-- 開啟UI
Window:Toggle(true)

-- 關閉UI
Window:Toggle(false)
```

### Window:Destroy()
完全銷毀並移除UI，釋放所有資源。此操作不可逆。

```lua
-- 銷毀UI（會清理所有線程、連接和GUI元素）
Window:Destroy()
```


## 建立左側標籤類別 
### DrawCategory 參數
```lua
-- 建立左側標籤類別
Window:DrawCategory({
    Name = "標籤名稱"
});
```

## 建立左側標籤 - 參數說明
### DrawTab 參數
| 參數              | 類型    | 說明                                                              |
| ----------------- | ------- | ----------------------------------------------------------------- |
| `Name`            | string  | 名稱                                                              |
| `Icon`            | string  | 名稱左邊圖示  來源:https://lucide.dev/icons                       |
| `Type`            | string  | "Single" 建立單一選項卡之標籤 、 "Double"建立雙選項卡之標籤(預設) |
| `EnableScrolling` | boolean | 是否啟用滾動功能（建議開啟，僅 Single 類型）                      |

```lua
-- 建立左側標籤
local NormalTab = Window:DrawTab({
    Name = "範例分頁",
    Icon = "apple",
    --Type = "Single",
});
```

## 建立右側選項卡 - 參數說明
### DrawSection 參數
| 參數          | 類型    | 說明                                             |
| ------------- | ------- | ------------------------------------------------ |
| `Name`        | string  | 名稱                                             |
| `Position`    | string  | 左邊選項卡(left)、右邊選項卡(right)              |
| `Canclose`    | boolean | 是否顯示關閉按鈕（預設為 true 顯示，false 隱藏） |
| `DefaultOpen` | boolean | 預設是否展開（預設為 true 展開，false 收合）      |

###### 注意! 建立單一選項卡之標籤 無法使用 Position 選項

```lua
-- 建立右側選項卡
local NormalSection = NormalTab:DrawSection({
    Name = "Section",
    Position = 'left'
});

-- 建立預設收合的選項卡
local ClosedSection = NormalTab:DrawSection({
    Name = "預設收合區塊",
    Position = 'left',
    Canclose = true,
    DefaultOpen = false  -- 預設收合
});
```

### DrawSection 方法

DrawSection 返回的物件支援以下方法來控制選項卡的顯示狀態：

#### SetVisible(bool)
控制 Section 的顯示/隱藏狀態。

| 參數   | 類型    | 說明                                  |
| ------ | ------- | ------------------------------------- |
| `bool` | boolean | `true` 顯示選項卡，`false` 隱藏選項卡 |

#### GetVisible()
獲取 Section 當前的顯示狀態。

| 返回值 | 類型    | 說明                                    |
| ------ | ------- | --------------------------------------- |
| -      | boolean | `true` 表示選項卡可見，`false` 表示隱藏 |

#### 使用範例

```lua
-- 建立 Section
local MySection = NormalTab:DrawSection({
    Name = "我的選項卡",
    Position = "left",
    Canclose = true
})

-- 隱藏 Section
MySection:SetVisible(false)

-- 顯示 Section
MySection:SetVisible(true)

-- 檢查 Section 是否可見
local isVisible = MySection:GetVisible()
print("Section 是否可見:", isVisible)

-- 在 Section 中添加元素
local Toggle = MySection:AddToggle({
    Name = "範例開關",
    Default = false,
    Callback = function(value)
        print("開關狀態:", value)
    end
})
```

## 建立右側容器選項卡 - 參數說明

### DrawContainerTab 參數
| 參數   | 類型   | 說明                                        |
| ------ | ------ | ------------------------------------------- |
| `Name` | string | 容器選項卡名稱                              |
| `Icon` | string | 名稱左邊圖示  來源:https://lucide.dev/icons |

#### 補充說明!! 在容器選項卡內建立DrawTab會是在上方
```lua
-- 建立右側容器選項卡
local ContainerTab = Window:DrawContainerTab({
	Name = "Extract Tabs",
	Icon = "contact",
});

-- 建立上方選項卡(雙選項卡)
local ExtractTab = ContainerTab:DrawTab({
	Name = "Tab 1",
	Type = "Double"
});

-- 建立上方選項卡(單一項卡)
local SingleExtractTab = ContainerTab:DrawTab({
	Name = "Tab 2",
	Type = "Single",
	EnableScrolling = true, -- 使標籤頁可以滾動（建議）
});
```

<!-- ======================================================================= -->
<!-- 以上為介面設置 往下為開關等功能性設置
-->

## 建立開關 - 參數說明
#### AddToggle 參數
| 參數                | 類型     | 必填 | 說明                             |
| ------------------- | -------- | ---- | -------------------------------- |
| `Name`              | string   | ✓    | 開關顯示名稱                     |
| `Default`           | boolean  | ✓    | 預設值                           |
| `Callback`          | function | ✓    | 值改變時執行的函數               |
| `ToggleColor`       | Color3   | ✗    | 自訂開關開啟時的顏色 (舊版相容)  |
| `ToggleColor_open`  | Color3   | ✗    | 自訂開關開啟時的顏色             |
| `ToggleColor_close` | Color3   | ✗    | 自訂開關關閉時的顏色             |
| `TextSize`          | number   | ✗    | 自訂文字大小                     |
| `Flag`              | string   | ✗    | 用於設定檔儲存的標記             |
| `Risky`             | boolean  | ✗    | 是否為高風險功能（顯示黃色文字） |

```lua
local AutoAttackToggle = NormalSection:AddToggle({
    Name = "自動攻擊",
    Default = false, -- 預設關閉
    Flag = "AutoAttack", -- 用於設定檔儲存
    ToggleColor_open = Color3.fromRGB(52, 199, 89), -- 開啟時顏色（綠色）
    ToggleColor_close = Color3.fromRGB(255, 0, 0), -- 關閉時顏色（紅色）
    TextSize = 16, -- 字體大小
    Risky = true, -- 高風險功能（顯示紅色文字）
    Callback = function(Value)
        if Value then
            print("自動攻擊：已開啟")
        else
            print("自動攻擊：已關閉")
        end
    end,
})
```

## 元素連結（.Link）- 參數說明
<figure><img src="https://2162411976-files.gitbook.io/~/files/v0/b/gitbook-x-prod.appspot.com/o/spaces%2Fu67EfmlwtAtmGvLQtoft%2Fuploads%2F1o36QgvPyJioWM7cDP6E%2Fimage.png?alt=media&#x26;token=9d4a885a-5ee6-4c8b-a3ec-ac63457311d7" alt=""><figcaption></figcaption></figure>
連結功能適用於 **開關 滑桿 快捷鍵 顏色選擇器**

### Link:AddHelper 參數
| 參數       | 類型   | 必填 | 說明         |
| ---------- | ------ | ---- | ------------ |
| `Text`     | string | ✓    | 輔助說明文字 |
| `TextSize` | number | ✗    | 文字大小     |

```lua
-- 連結輔助說明到元素
Element.Link:AddHelper({
    Text = "輔助說明文字",
    TextSize = 16
})
```

### Link:AddOption 參數

<figure><img src="https://2162411976-files.gitbook.io/~/files/v0/b/gitbook-x-prod.appspot.com/o/spaces%2Fu67EfmlwtAtmGvLQtoft%2Fuploads%2FCownbqOE9C24uj0xQYwY%2Fimage.png?alt=media&#x26;token=19960847-fce3-41d2-a9ae-956f90361440" alt=""><figcaption></figcaption></figure>
「選項」就像是「區塊」但在「元素」內，可在選項內新增更多元素。

```lua
-- 連結選項到元素
local Option = Toggle.Link:AddOption()

-- 在選項內新增開關
Option:AddToggle({
    Name = "選項內的開關！",
    Default = false,
    ToggleColor = Color3.fromRGB(255, 100, 100),
    Callback = print
})
```

#### AddOption 支援的元素

| 元素類型    | 說明                 |
| ----------- | -------------------- |
| `AddToggle` | 在選項內新增開關     |
| 其他元素    | 支援所有標準元素類型 |
<!--

### Link:AddKeybind 參數
| 參數       | 類型     | 必填 | 說明                 |
| ---------- | -------- | ---- | -------------------- |
| `Default`  | string   | ✗    | 預設快捷鍵           |
| `Flag`     | string   | ✗    | 用於設定檔儲存的標記 |
| `Callback` | function | ✗    | 值改變時執行的函數   |

```lua
-- 連結快捷鍵到元素
Element.Link:AddKeybind({
    Default = "E",
    Flag = "Option_Keybind",
    Callback = print
});
```

### Link:AddColorPicker 參數
| 參數       | 類型     | 必填 | 說明                 |
| ---------- | -------- | ---- | -------------------- |
| `Default`  | Color3   | ✗    | 預設顏色             |
| `Flag`     | string   | ✗    | 用於設定檔儲存的標記 |
| `Callback` | function | ✗    | 值改變時執行的函數   |

```lua
-- 連結顏色選擇器到元素
Element.Link:AddColorPicker({
    Default = Color3.fromRGB(255,255,255),
    Flag = "Color_Option",
    Callback = print
})
```
-->

## 建立滑桿 - 參數說明
#### AddSlider 參數
| 參數       | 類型     | 必填 | 說明                       |
| ---------- | -------- | ---- | -------------------------- |
| `Name`     | string   | ✓    | 滑桿顯示名稱               |
| `Min`      | number   | ✓    | 最小值                     |
| `Max`      | number   | ✓    | 最大值                     |
| `Default`  | number   | ✓    | 預設值（需在Min與Max之間） |
| `Callback` | function | ✓    | 值改變時執行的函數         |
| `Round`    | number   | ✗    | 數值精確度（小數點位數）   |
| `Flag`     | string   | ✗    | 用於設定檔儲存的標記       |

```lua
-- 基本滑桿範例
NormalSection:AddSlider({
    Name = "移動速度",
    Min = 0,
    Max = 100,
    Default = 50,
    Round = 0, -- 整數，不顯示小數
    Flag = "Slider_Example",
    Callback = function(Value)
        print("當前速度:", Value)
    end
});

-- 帶小數的滑桿範例
NormalSection:AddSlider({
    Name = "精確控制",
    Min = 0,
    Max = 1,
    Default = 0.5,
    Round = 2, -- 保留兩位小數
    Flag = "Precise_Slider",
    Callback = function(Value)
        print("精確值:", Value)
    end
});
```

## 建立顏色選擇器 - 參數說明
#### AddColorPicker 參數
| 參數       | 類型     | 必填 | 說明                 |
| ---------- | -------- | ---- | -------------------- |
| `Name`     | string   | ✓    | 名稱                 |
| `Default`  | Color3   | ✓    | 預設值（Color3）     |
| `Callback` | function | ✓    | 值改變時執行的函數   |
| `Flag`     | string   | ✗    | 用於設定檔儲存的標記 |
```lua
NormalSection:AddColorPicker({
    Name = "ColorPicker",
    Default = Color3.fromRGB(0, 255, 140),
    Flag = "Color_Picker_Example",
    Callback = print
})
```

## 建立下拉選單 - 參數說明
#### AddDropdown 參數
| 參數       | 類型         | 必填 | 說明                 |
| ---------- | ------------ | ---- | -------------------- |
| `Name`     | string       | ✓    | 名稱                 |
| `Default`  | string       | ✓    | 文字或表格           |
| `Multi`    | Boolean      | ✓    | 用來控制是否可以多選 |
| `Values`   | string(list) | ✓    | 選項值               |
| `Callback` | function     | ✓    | 值改變時執行的函數   |
| `Flag`     | string       | ✗    | 用於設定檔儲存的標記 |

```lua
NormalSection:AddDropdown({
    Name = "Single Dropdown",
    Default = "Head",
    Flag = "Single_Dropdown",
    Values = {"Head","Body","Arms","Legs"},
    Callback = print
})

NormalSection:AddDropdown({
    Name = "Multi Dropdown",
    Default = {"Head"},
    Multi = true,
    Flag = "Multi_Dropdown",
    Values = {"Head","Body","Arms","Legs"},
    Callback = print
})
```

### 動態修改下拉選單

下拉選單支援以下方法來動態修改其內容和狀態：

#### 可用方法

| 方法                | 參數   | 說明                                  |
| ------------------- | ------ | ------------------------------------- |
| `SetText(str)`      | string | 修改下拉選單的標題文字                |
| `GetText()`         | -      | 獲取當前標題文字                      |
| `SetValues(values)` | table  | 修改下拉選單的選項列表                |
| `SetValue(value)`   | value  | 強制設定選中的項目（會觸發 Callback） |
| `GetValue()`        | -      | 獲取當前選中的值                      |

#### 使用範例

```lua
-- 建立下拉選單並保存引用
local dropdown = NormalSection:AddDropdown({
	Name = "選擇武器",
	Default = "劍",
	Values = {"劍", "槍", "弓"},
	Flag = "Weapon_Dropdown",
	Callback = function(value)
		print("選擇了:", value)
	end
})

-- 修改標題
dropdown:SetText("主要武器選擇")

-- 動態更新選項列表
task.wait(2)
dropdown:SetValues({"劍", "槍", "弓", "魔杖", "匕首"})

-- 程式強制選擇（會觸發 Callback）
dropdown:SetValue("魔杖")

-- 獲取當前選中的值
local currentWeapon = dropdown:GetValue()
print("當前武器:", currentWeapon)

-- 獲取當前標題
local title = dropdown:GetText()
print("標題:", title)
```

#### 多選模式範例

```lua
local multiDropdown = NormalSection:AddDropdown({
	Name = "選擇技能",
	Default = {"火球術"},
	Multi = true,
	Values = {"火球術", "冰凍術", "治療術", "閃電術"},
	Callback = function(selected)
		for skill, enabled in pairs(selected) do
			if enabled then
				print("已啟用:", skill)
			end
		end
	end
})

-- 修改選項（新增更多技能）
multiDropdown:SetValues({"火球術", "冰凍術", "治療術", "閃電術", "風暴術", "傳送術"})

-- 設定多個選中項目（會觸發 Callback）
multiDropdown:SetValue({["火球術"] = true, ["治療術"] = true})
```

## 建立按鈕 - 參數說明
#### AddButton 參數
| 參數               | 類型     | 必填 | 說明               |
| ------------------ | -------- | ---- | ------------------ |
| `Name`             | string   | ✓    | 名稱               |
| `Callback`         | function | ✓    | 值改變時執行的函數 |
| `TextColor3`       | Color3   | ✗    | 按鈕字體顏色       |
| `BackgroundColor3` | Color3   | ✗    | 按鈕背景顏色       |

```lua
NormalSection:AddButton({
    Name = "完全自訂按鈕",
    TextColor3 = Color3.fromRGB(255, 255, 255),
    BackgroundColor3 = Color3.fromRGB(100, 50, 200),
    Callback = function()
        print('完全自訂按鈕被點擊!')
    end,
})
```

## 建立按鈕 - 參數說明
#### AddButton 參數
| 參數       | 類型     | 必填 | 說明               |
| ---------- | -------- | ---- | ------------------ |
| `Name`     | string   | ✓    | 按鈕文字           |
| `TextSize` | number   | ✗    | 文字大小（預設12） |
| `Callback` | function | ✗    | 點擊回調函數       |

```lua
NormalSection:AddButton({
    Name = "Button",
    TextSize = 14,
    Callback = function()
        print("Button clicked!")
    end
})
```

## 建立並排按鈕 - 參數說明
#### AddButtonRow 參數
這個功能允許你在一行中並排顯示多個按鈕，非常適合需要將多個相關操作組合在一起的情況。

| 參數      | 類型    | 必填 | 說明         |
| --------- | ------- | ---- | ------------ |
| `Buttons` | {table} | ✓    | 按鈕配置陣列 |

#### Buttons 陣列中每個按鈕的參數
| 參數               | 類型     | 必填 | 說明                         |
| ------------------ | -------- | ---- | ---------------------------- |
| `Name`             | string   | ✓    | 按鈕文字                     |
| `TextColor3`       | Color3   | ✗    | 文字顏色（預設使用主題顏色） |
| `BackgroundColor3` | Color3   | ✗    | 背景顏色（預設使用主題顏色） |
| `TextSize`         | number   | ✗    | 文字大小（預設12）           |
| `Callback`         | function | ✗    | 點擊回調函數                 |

```lua
-- 2個按鈕並排
NormalSection:AddButtonRow({
    Buttons = {
        {
            Name = "按鈕 1",
            BackgroundColor3 = Color3.fromRGB(255, 100, 100),
            Callback = function()
                print("按鈕 1")
            end
        },
        {
            Name = "按鈕 2",
            BackgroundColor3 = Color3.fromRGB(100, 255, 100),
            Callback = function()
                print("按鈕 2")
            end
        }
    }
})

-- 3個按鈕並排
NormalSection:AddButtonRow({
    Buttons = {
        {
            Name = "Yes",
            TextColor3 = Color3.fromRGB(255, 255, 255),
            BackgroundColor3 = Color3.fromRGB(34, 197, 94),
            TextSize = 14,
            Callback = function()
                print("確認")
            end
        },
        {
            Name = "No",
            TextColor3 = Color3.fromRGB(255, 255, 255),
            BackgroundColor3 = Color3.fromRGB(239, 68, 68),
            TextSize = 14,
            Callback = function()
                print("取消")
            end
        },
        {
            Name = "Maybe",
            TextColor3 = Color3.fromRGB(0, 0, 0),
            BackgroundColor3 = Color3.fromRGB(250, 204, 21),
            TextSize = 14,
            Callback = function()
                print("不確定")
            end
        }
    }
})
```

## 建立標籤文字 - 參數說明
#### AddLabel 參數
| 參數         | 類型    | 必填 | 說明                                         |
| ------------ | ------- | ---- | -------------------------------------------- |
| `Text`       | string  | ✓    | 顯示的文字內容                               |
| `TextColor3` | Color3  | ✗    | 文字顏色（預設使用主題顏色）                 |
| `TextSize`   | number  | ✗    | 文字大小（預設14）                           |
| `RichText`   | boolean | ✗    | 是否啟用富文本（支援粗體、斜體等，預設啟用） |

```lua
-- 基本用法
NormalSection:AddLabel({
    Text = "這是一個簡單的標籤"
})

-- 自訂顏色和大小
local myLabel = NormalSection:AddLabel({
    Text = "這是紅色的標籤",
    TextColor3 = Color3.fromRGB(255, 100, 100),
    TextSize = 16
})

-- 使用富文本
NormalSection:AddLabel({
    Text = "<b>粗體</b> 和 <i>斜體</i>",
    TextColor3 = Color3.fromRGB(100, 200, 255),
    RichText = true
})

-- 動態更新
myLabel:SetText("更新的文字")
myLabel:SetColor(Color3.fromRGB(0, 255, 0))
myLabel:SetTextSize(18)
```

#### AddLabel 方法
| 方法                | 參數   | 說明         |
| ------------------- | ------ | ------------ |
| `SetText(text)`     | string | 更新文字內容 |
| `SetColor(color)`   | Color3 | 更改文字顏色 |
| `SetTextSize(size)` | number | 調整文字大小 |

## 建立段落文字 - 參數說明
#### AddParagraph 參數
| 參數        | 類型               | 必填 | 說明                             |
| ----------- | ------------------ | ---- | -------------------------------- |
| `Title`     | string             | ✓    | 標題(大字體)                     |
| `Content`   | string 或 {string} | ✗    | 內文(小字體)，支援字串或字串表格 |
| `TitleSize` | number             | ✗    | 標題文字大小(預設14)             |
```lua
-- 基本用法
NormalSection:AddParagraph({
    Title = "Paragraph",
    Content = "Very cool paragraph"
})

-- 使用表格傳入多行內容
NormalSection:AddParagraph({
    Title = "多行段落",
    Content = {
        "第一行內容",
        "第二行內容",
        "第三行內容"
    }
})

-- 自訂文字大小
NormalSection:AddParagraph({
    Title = "大標題",
    Content = {
        "內容行 1",
        "內容行 2"
    },
    TitleSize = 16,
    ContentSize = 14
})

-- 自訂間隔
NormalSection:AddParagraph({
    Title = "大行間距段落",
    Content = {
        "第一行",
        "第二行",
        "第三行"
    },
    Spacing = 1.5  -- 增加行間距至 1.5 倍
})
```

## 建立文字輸入框 - 參數說明
#### AddInputText 參數

全寬文字輸入框元件，佈局為上方標題 + 下方全寬輸入框，含清除按鈕（X）。

| 參數          | 類型     | 必填 | 說明                           |
| ------------- | -------- | ---- | ------------------------------ |
| `Name`        | string   | ✓    | 輸入框標題                     |
| `Default`     | string   | ✗    | 預設值（預設為空字串）         |
| `Placeholder` | string   | ✗    | 佔位符文字                     |
| `Callback`    | function | ✓    | 文字變更時執行的函數           |
| `MaxLength`   | number   | ✗    | 最大字元長度（不填則無限制）   |
| `Flag`        | string   | ✗    | 用於設定檔儲存的標記           |

```lua
-- 基本文字輸入
local nameInput = NormalSection:AddInputText({
    Name = "玩家暱稱",
    Default = "",
    Placeholder = "請輸入暱稱...",
    Flag = "PlayerName",
    Callback = function(text)
        print("輸入的暱稱:", text)
    end
})

-- 帶最大長度限制
NormalSection:AddInputText({
    Name = "驗證碼",
    Default = "",
    Placeholder = "輸入6位驗證碼",
    MaxLength = 6,
    Flag = "VerifyCode",
    Callback = function(text)
        print("驗證碼:", text)
    end
})
```

#### AddInputText 方法
| 方法               | 參數   | 說明                       |
| ------------------ | ------ | -------------------------- |
| `SetText(str)`     | string | 修改標題文字               |
| `GetText()`        | -      | 獲取當前標題文字           |
| `SetValue(value)`  | string | 設定輸入值（會觸發回呼）   |
| `GetValue()`       | -      | 獲取當前輸入值             |
| `SetVisible(bool)` | boolean| 控制顯示/隱藏              |
| `GetVisible()`     | -      | 獲取當前顯示狀態           |

```lua
-- 動態操作
nameInput:SetValue("新名稱")
print(nameInput:GetValue()) -- 輸出: 新名稱
nameInput:SetText("角色名稱")
```

## 建立數字輸入框 - 參數說明
#### AddInputNum 參數

全寬數字輸入框元件，含 `-/+` 步進按鈕，支援數值範圍限制和小數控制。

| 參數          | 類型     | 必填 | 說明                              |
| ------------- | -------- | ---- | --------------------------------- |
| `Name`        | string   | ✓    | 輸入框標題                        |
| `Default`     | number   | ✗    | 預設數值（預設為 0）              |
| `Placeholder` | string   | ✗    | 佔位符文字                        |
| `Callback`    | function | ✓    | 數值變更時執行的函數              |
| `Min`         | number   | ✗    | 最小值（不填則無下限）            |
| `Max`         | number   | ✗    | 最大值（不填則無上限）            |
| `Step`        | number   | ✗    | +/- 按鈕的步進值（預設為 1）      |
| `Round`       | number   | ✗    | 小數位數（預設為 0，即整數）      |
| `Flag`        | string   | ✗    | 用於設定檔儲存的標記              |

```lua
-- 基本數字輸入
local atkInput = NormalSection:AddInputNum({
    Name = "攻擊力",
    Default = 100,
    Min = 0,
    Max = 9999,
    Step = 10,
    Flag = "AttackPower",
    Callback = function(value)
        print("攻擊力:", value)
    end
})

-- 帶小數的精確數字輸入
NormalSection:AddInputNum({
    Name = "倍率",
    Default = 1.0,
    Min = 0.1,
    Max = 10.0,
    Step = 0.1,
    Round = 1, -- 保留一位小數
    Flag = "Multiplier",
    Callback = function(value)
        print("倍率:", value)
    end
})
```

#### AddInputNum 方法
| 方法               | 參數   | 說明                                  |
| ------------------ | ------ | ------------------------------------- |
| `SetText(str)`     | string | 修改標題文字                          |
| `GetText()`        | -      | 獲取當前標題文字                      |
| `SetValue(value)`  | number | 設定數值（會自動 clamp 並觸發回呼）   |
| `GetValue()`       | -      | 獲取當前數值                          |
| `SetMin(val)`      | number | 動態修改最小值（會重新限制當前值）    |
| `SetMax(val)`      | number | 動態修改最大值（會重新限制當前值）    |
| `SetVisible(bool)` | boolean| 控制顯示/隱藏                         |
| `GetVisible()`     | -      | 獲取當前顯示狀態                      |

```lua
-- 動態操作
atkInput:SetValue(500)       -- 設定為 500
print(atkInput:GetValue())   -- 輸出: 500
atkInput:SetMax(5000)        -- 修改上限為 5000
atkInput:SetMin(100)         -- 修改下限為 100
atkInput:SetText("基礎攻擊")
```

<!--= ->
<!-- 額外功能
= -->

## 元件通用方法

所有 UI 元件都支援以下通用方法來控制其顯示狀態。這讓你可以根據條件動態顯示或隱藏元件。

### SetVisible - 控制元件顯示/隱藏

**功能說明：** 設置元件的可見性

**參數：**
| 參數   | 類型    | 說明                              |
| ------ | ------- | --------------------------------- |
| `bool` | boolean | `true` 顯示元件，`false` 隱藏元件 |

**返回值：** 無

```lua
-- 隱藏元件
element:SetVisible(false)

-- 顯示元件
element:SetVisible(true)
```

### GetVisible - 獲取元件顯示狀態

**功能說明：** 獲取元件當前的可見性狀態

**參數：** 無

**返回值：** boolean - 元件當前是否可見

```lua
local isVisible = element:GetVisible()
print("元件是否顯示:", isVisible)
```

### 完整範例

```lua
-- 建立主開關
local AutoFarmToggle = Section:AddToggle({
    Name = "自動農場",
    Default = false,
    Callback = function(value)
        print("自動農場:", value)
    end
})

-- 建立條件開關
local ShowAdvancedToggle = Section:AddToggle({
    Name = "顯示進階設定",
    Default = false,
    Callback = function(value)
        -- 根據條件顯示/隱藏其他元件
        FarmSpeedSlider:SetVisible(value)
        FarmModeDropdown:SetVisible(value)
        AdvancedSection:SetVisible(value)
    end
})

-- 建立進階元件（預設隱藏）
local FarmSpeedSlider = Section:AddSlider({
    Name = "農場速度",
    Min = 1,
    Max = 10,
    Default = 5,
    Callback = function(value)
        print("速度:", value)
    end
})
FarmSpeedSlider:SetVisible(false) -- 預設隱藏

local FarmModeDropdown = Section:AddDropdown({
    Name = "農場模式",
    Default = "普通",
    Values = {"普通", "快速", "超快"},
    Callback = function(value)
        print("模式:", value)
    end
})
FarmModeDropdown:SetVisible(false) -- 預設隱藏

-- 也可以控制整個 Section 的顯示
local AdvancedSection = Tab:AddSection({
    Name = "進階設定"
})
AdvancedSection:SetVisible(false) -- 預設隱藏整個區塊
```

### 動態顯示範例

```lua
-- 根據玩家等級顯示不同元件
local playerLevel = 15

local BeginnerSection = Tab:AddSection({ Name = "新手設定" })
local AdvancedSection = Tab:AddSection({ Name = "進階設定" })
local ExpertSection = Tab:AddSection({ Name = "專家設定" })

-- 控制 Section 顯示
BeginnerSection:SetVisible(playerLevel < 10)
AdvancedSection:SetVisible(playerLevel >= 10 and playerLevel < 50)
ExpertSection:SetVisible(playerLevel >= 50)

-- 根據遊戲狀態控制元件
local inCombat = false

game:GetService("RunService").Heartbeat:Connect(function()
    local currentInCombat = (玩戲狀態檢查函數)
    
    if currentInCombat ~= inCombat then
        inCombat = currentInCombat
        -- 戰鬥中隱藏某些元件，避免干擾
        SafeSettingsSection:SetVisible(not inCombat)
    end
end)
```

### 支援 SetVisible 的元件列表

以下所有元件都完整支援 `SetVisible()` 和 `GetVisible()` 方法：

| 元件類型           | 說明       |
| ------------------ | ---------- |
| **Section**        | 區塊       |
| **AddToggle**      | 開關       |
| **AddSlider**      | 滑桿       |
| **AddButton**      | 按鈕       |
| **AddButtonRow**   | 並排按鈕   |
| **AddDropdown**    | 下拉選單   |
| **AddKeybind**     | 快捷鍵     |
| **AddColorPicker** | 顏色選擇器 |
| **AddLabel**       | 標籤文字   |
| **AddParagraph**   | 段落文字   |
| **AddTextBox**     | 文字輸入框 |
| **AddInputText**   | 全寬文字輸入框 |
| **AddInputNum**    | 數字輸入框 |

### 特性說明

- ✅ **即時生效**：`SetVisible()` 直接設置元件的 `Visible` 屬性，立即生效，無動畫延遲
- ✅ **狀態保留**：隱藏的元件不會顯示在介面上，但仍保留其值和狀態
- ✅ **可查詢**：可透過 `GetVisible()` 檢查元件當前的顯示狀態
- ✅ **層級控制**：可以控制單個元件或整個 Section 的顯示
- ✅ **動態控制**：可在執行時隨時改變元件的可見性

### 注意事項

1. **初始狀態**：元件建立後預設為顯示狀態，如需隱藏請在建立後立即調用 `SetVisible(false)`
2. **Section 控制**：隱藏 Section 會同時隱藏內部所有元件
3. **效能考量**：頻繁切換大量元件的可見性可能影響效能，建議批次處理或使用 Section 層級控制

## 建立配置管理器 - 參數說明

### ConfigManager 參數
| 參數        | 類型   | 必填 | 說明             |
| ----------- | ------ | ---- | ---------------- |
| `Directory` | string | ✓    | 工作區中的資料夾 |
| `Config`    | string | ✓    | 腳本的設定資料夾 |

```lua
-- 建立配置管理器
local ConfigManager = Compkiller:ConfigManager({
    Directory = "Compkiller-UI",
    Config = "Example-Configs",
});
```

### 配置管理器功能

#### 寫入設定
將當前數值寫入設定檔

```lua
ConfigManager:WriteConfig({
    Name = "設定名稱",
    Author = "建立者名稱"
})
```

#### 讀取資訊
讀取設定檔資訊，例如：建立者名稱和建立日期

```lua
ConfigManager:ReadInfo("<設定名稱>")
```

#### 載入設定
載入目錄中的設定檔

```lua
ConfigManager:LoadConfig("<設定名稱>")
```

#### 刪除設定
刪除目錄中的設定檔

```lua
ConfigManager:DeleteConfig("<設定名稱>")
```

#### 取得所有設定
取得目錄中的所有設定檔

```lua
ConfigManager:GetConfigs()
```

#### 目錄路徑
設定檔目錄路徑

```lua
ConfigManager.Directory
```

## 建立設定分頁 - 參數說明

### DrawConfig 參數
| 參數     | 類型   | 必填 | 說明                                                    |
| -------- | ------ | ---- | ------------------------------------------------------- |
| `Name`   | string | ✓    | 設定分頁名稱                                            |
| `Icon`   | string | ✓    | 名稱左邊圖示  來源:https://lucide.dev/icons             |
| `Config` | object | ✓    | 設定管理器物件（使用 `ConfigManager` 建立的管理器物件） |

```lua
-- 建立設定分頁
local ConfigUI = Window:DrawConfig({
    Name = "設定",
    Icon = "folder",
    Config = ConfigManager, -- 設定管理器（必填）
});

-- 初始化設定介面
ConfigUI:Init();
```

<figure><img src="https://2162411976-files.gitbook.io/~/files/v0/b/gitbook-x-prod.appspot.com/o/spaces%2Fu67EfmlwtAtmGvLQtoft%2Fuploads%2FVtNqgtwn3s9HPdsPq6NT%2Fimage.png?alt=media&#x26;token=0b0e53b5-928b-4385-920f-d8e1a549ea8f" alt=""><figcaption><p>設定分頁預覽</p></figcaption></figure>

