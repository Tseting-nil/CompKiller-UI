...
local _screenSize = CurrentCamera.ViewportSize
local _isMobilePlatform = (_platform == Enum.Platform.Android or _platform == Enum.Platform.IOS)

-- 判斷是否為「手機尺寸」（平板 / WSA / PC 都算大介面）
-- 螢幕寬度 < 900 且為手機平台才使用小介面
local useSmallUI = _isMobilePlatform and _screenSize.X < 900

if useSmallUI then
	print("[Compkiller] 📱 手機端 - 使用小介面")
else
	print("[Compkiller] 💻 電腦 / 平板 / WSA - 使用大介面")
end
...
local Compkiller = {
	...
	Windows = {},
	Scale = {
		Window = UDim2.new(0, 485, 0, 565),    -- 電腦/平板：485x565
		Mobile = UDim2.new(0, 320, 0, 280),    -- 手機：320x280（縮小以避免佔滿螢幕）
		TabOpen = 185,
		TabClose = 85,
	},
...
Compkiller.Colors = {
	Highlight = Color3.fromRGB(203, 127, 55),
...
function Compkiller:_Blur(element : Frame , WindowRemote) : RBXScriptSignal
	...
	local THREAD = task.spawn(function()
		while true do task.wait(0.033) -- 約 30 FPS 更新，優化性能（原本 0.01）
			pcall(UpdateFunction);
		end;
	end);
...
function Compkiller:Drag(InputFrame: Frame, MoveFrame: Frame, Speed : number)
	...
	local currentTween = nil; -- 追蹤當前的 Tween，用於取消殘影
	...
	local updateInput = function(input)
		...
		currentTween = Compkiller:_Animation(MoveFrame,Tween,{
			Position = position
		});
	end;
...
function Compkiller:_IsMobile()
	-- 使用預先計算的 useSmallUI 變數
	-- 只有手機平台 + 小螢幕才返回 true
	return useSmallUI;
end;

function Compkiller:_UseSmallUI()
	return useSmallUI;
end;

function Compkiller:_RefreshUISize()
	-- 重新計算螢幕尺寸（用於螢幕旋轉或視窗大小變化時）
	local camera = workspace.CurrentCamera
	local screenSize = camera.ViewportSize
	local platform = UserInputService:GetPlatform()
	
	local isMobilePlatform = (platform == Enum.Platform.Android or platform == Enum.Platform.IOS)
	useSmallUI = isMobilePlatform and screenSize.X < 900
		...
		local ToggleElement = function(bool,noChange)
			...
			if bool then
				...
				Compkiller:_Animation(Toggle,rep.Tween,{
					BackgroundColor3 = ToggleColor or Compkiller.Colors.Toggle
				})
			else
...
function Compkiller:PreloadIcons()
	...
	if Compkiller.SecureMode then
		for i,v in next , RequiredAssets do -- 移除 task.wait() 優化性能
			pcall(function()
...
function Compkiller:_AddColorPickerPanel(Button: ImageButton , Callback: (Color: Color3) -> any?)
	...
	TransparentImage.ImageColor3 = Color3.fromRGB(255, 255, 255)
	...
	ColorPickerWindow.InputBegan:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
			...
			SPAWN_THREAD = task.spawn(function()
				while true do task.wait(0.016) -- ~60 FPS，優化性能（原本 0.00001）
	...
	ColorRedGreenBlue.InputBegan:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
			...
			while (UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) or Args.IsHold) do task.wait(0.016) -- ~60 FPS
...
function Compkiller:_LoadDropdown(BaseParent: TextButton , Callback: () -> any)
	...
	BaseParent.MouseButton1Click:Connect(function()
		...
		-- 切換開關狀態：如果已打開則關閉，如果已關閉則打開
		local isCurrentlyOpen = ToggleDb:GetValue();
...
function Compkiller:_LoadElement(Parent: Frame , EnabledLine: boolean , Signal , DisableStackKeybind)
	...
	function Args:AddButtonRow(Config: ButtonRow)
		Config = Compkiller.__CONFIG(Config, {
			Buttons = {}
		});
		...
		if Compkiller:_IsMobile() then
			Compkiller:_AddDragBlacklist(ButtonRowFrame);
		end;

		ButtonRowFrame.Name = Compkiller:_RandomString()
		ButtonRowFrame.Parent = Parent
		ButtonRowFrame.BackgroundTransparency = 1.000
		ButtonRowFrame.BorderColor3 = Color3.fromRGB(0, 0, 0)
		ButtonRowFrame.BorderSizePixel = 0
		ButtonRowFrame.Size = UDim2.new(1, -1, 0, 30)
		ButtonRowFrame.ZIndex = Zindex + 5

		BlockLine.Name = Compkiller:_RandomString()
		BlockLine.Parent = ButtonRowFrame
		BlockLine.AnchorPoint = Vector2.new(0.5, 1)
		BlockLine.BackgroundColor3 = Compkiller.Colors.LineColor
		BlockLine.BackgroundTransparency = 0.500
		BlockLine.BorderColor3 = Color3.fromRGB(0, 0, 0)
		...
		ButtonContainer.Name = Compkiller:_RandomString()
		ButtonContainer.Parent = ButtonRowFrame
		...
		ButtonContainer.Size = UDim2.new(1, -15, 1, -5)
		ButtonContainer.ZIndex = Zindex + 7
		...
		for i, buttonConfig in ipairs(Config.Buttons) do
			...
			Frame.BackgroundColor3 = buttonConfig.BackgroundColor3 or Compkiller.Colors.Highlight
			Frame.BackgroundTransparency = 0.100
			Frame.BorderColor3 = Color3.fromRGB(0, 0, 0)
			Frame.BorderSizePixel = 0
			Frame.Size = UDim2.new(buttonWidth, -5 * (buttonCount - 1) / buttonCount, 1, 0)
			Frame.ZIndex = Zindex + 8
			Frame.LayoutOrder = i

			if not buttonConfig.BackgroundColor3 then
				table.insert(Compkiller.Elements.Highlight, {
					Element = Frame,
					Property = "BackgroundColor3"
				});
			...
			TextLabel.TextColor3 = buttonConfig.TextColor3 or Compkiller.Colors.SwitchColor
			TextLabel.TextSize = buttonConfig.TextSize or 12
			TextLabel.TextStrokeTransparency = 0.900

			if not buttonConfig.TextColor3 then
				table.insert(Compkiller.Elements.SwitchColor, {
					Element = TextLabel,
					Property = 'TextColor3'
				});
		...
		ReturnArgs.Signal = Signal:Connect(function(bool)
			if bool then
	...
	function Args:AddButton(Config: Button)
		Config = Compkiller.__CONFIG(Config , {
			Name = 'Button',
			TextColor3 = nil, -- 可選的自訂按鈕文字顏色
			BackgroundColor3 = nil, -- 可選的自訂按鈕背景顏色
			TextSize = 12, -- 可選的自訂按鈕文字大小
			Callback = function() end
		});
		...
		-- 如果有提供 BackgroundColor3 參數則使用，否則使用預設的 Highlight
		Frame.BackgroundColor3 = Config.BackgroundColor3 or Compkiller.Colors.Highlight
		Frame.BackgroundTransparency = 0.100
		Frame.BorderColor3 = Color3.fromRGB(0, 0, 0)
		Frame.BorderSizePixel = 0
		...
		-- 只有在沒有自訂背景顏色的情況下才加入全域顏色追蹤
		if not Config.BackgroundColor3 then
			table.insert(Compkiller.Elements.Highlight,{
				Element = Frame,
				Property = "BackgroundColor3"
			});
		...
		-- 如果有提供 Color3 參數則使用，否則使用預設的 SwitchColor
		TextLabel.TextColor3 = Config.TextColor3 or Compkiller.Colors.SwitchColor
		TextLabel.TextSize = Config.TextSize
		TextLabel.TextStrokeTransparency = 0.900

		-- 只有在沒有自訂顏色的情況下才加入全域顏色追蹤
	...
	function Args:AddLabel(Config: Label)
		...
		if Compkiller:_IsMobile() then
			Compkiller:_AddDragBlacklist(LabelFrame);
		end;
		...
		LabelFrame.Size = UDim2.new(1, -1, 0, 30)
		LabelFrame.ZIndex = Zindex + 2
		LabelFrame.ClipsDescendants = true

		LabelText.Name = Compkiller:_RandomString()
		LabelText.Parent = LabelFrame
		LabelText.AnchorPoint = Vector2.new(0, 0.5)
		LabelText.BackgroundTransparency = 1.000
		LabelText.BorderColor3 = Color3.fromRGB(0, 0, 0)
		LabelText.BorderSizePixel = 0
		LabelText.Position = UDim2.new(0, 12, 0.5, 0)
		LabelText.Size = UDim2.new(1, -20, 1, -10)
		...
		local UpdateScale = function()
			local TextScale = TextService:GetTextSize(
				LabelText.Text,
				LabelText.TextSize,
				LabelText.Font,
				Vector2.new(LabelFrame.AbsoluteSize.X - 24, math.huge)
			);

			local newHeight = math.max(30, TextScale.Y + 15);
			Compkiller:_Animation(LabelFrame, TweenInfo.new(0.15), {
				Size = UDim2.new(1, -1, 0, newHeight)
			});
		end;
		...
		function ReturnArgs:SetText(text)
			LabelText.Text = text;
			UpdateScale();
		end;

		function ReturnArgs:SetColor(color: Color3)
			LabelText.TextColor3 = color;
		end;

		function ReturnArgs:SetTextSize(size: number)
		...
		ReturnArgs.Signal = Signal:Connect(function(bool)
			if bool then
				Compkiller:_Animation(LabelText, TweenInfo.new(0.2), {
					TextTransparency = 0.300
				});
	...
	function Args:AddParagraph(Config: Paragraph) -- request by Neptune
		...
		if type(Config.Content) == "table" then
			contentText = table.concat(Config.Content, "\n")
		elseif type(Config.Content) == "string" then
			contentText = Config.Content
		end;
		...
		BlockText.TextColor3 = Compkiller.Colors.SwitchColor
		BlockText.TextSize = Config.TitleSize
		BlockText.TextTransparency = 0.300
		BlockText.TextXAlignment = Enum.TextXAlignment.Left
		...
		DescriptionText.Text = contentText
		DescriptionText.TextColor3 = Compkiller.Colors.SwitchColor
		DescriptionText.TextSize = Config.ContentSize
		...
		DescriptionText.LineHeight = Config.Spacing
...
function Compkiller.new(Config : Window)
	...
	CompLogo.Visible = ShowLogo -- 根據 ShowLogo 參數控制 Logo 是否顯示
...
-- 	
	...
	WindowLabel.Position = ShowLogo and UDim2.new(0, 60, 0, 17) or UDim2.new(0, 15, 0, 7) -- 根據 ShowLogo 調整位置
	...
	TabHover:Connect(function(value)
		...
		if value then
			...
			Compkiller:_Animation(WindowLabel , Style , {
				Position = ShowLogo and UDim2.new(0, 60, 0, 17) or UDim2.new(0, 15, 0, 7),
				TextTransparency = 0
			});
	...
	do
		...
		function WindowArgs.UserSettings:Create()
			...
			-- WindowArgs.UserSettings.Signal = Compkiller:_Blur(UserSettings,Signal); -- 移除模糊層效果
	...
	function WindowArgs:DrawTab(TabConfig : TabConfig , Internal)
		...
		function TabArgs:_UpdateScrolling(Frame: ScrollingFrame , ListLayout: UIListLayout)
			...
			for i,v in next ,Childrens do -- 移除 task.wait() 優化性能
				if v:IsA('Frame') then
		...
		function TabArgs:DrawSection(config: Section)
			...
			SectionClose.Image = Compkiller:CacheImage("rbxassetid://109535175596957")
			SectionClose.ImageTransparency = 0.500
			SectionClose.Visible = config.Canclose
			...
			local refreshScale = function()
				...
				for i,v: Frame in next , Childrens do -- 移除 task.wait() 優化性能
					if v:IsA('Frame') then
						if v ~= Section then
							frameFound += 1;
